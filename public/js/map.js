...
async function initMap() {
    ...
    locationButton.addEventListener("click", () => {
      if (navigator.geolocation) { //Geolocation API(デバイスの位置情報をやり取りするシステム)をサポートしているかをnavigator.geolocationオブジェクトの有無で確認
        navigator.geolocation.getCurrentPosition((position) => { //getCurrentPosition()で、現在地を取得する非同期通信を開始し、第1引数に成功した場合に実行されるコールバック関数を記述
          const pos = { //{}はキーと値(key: value)で構成されるオブジェクトを指す
      ...
      } else { //Geolocation APIをサポートしていない場合の処理
        handleLocationError(false, infoWindow, map.getCenter());
      }
    });

    function handleLocationError(browserHasGeolocation, infoWindow, pos) { //browserHasGeolocationにtrueかfalseの値を持たせる
